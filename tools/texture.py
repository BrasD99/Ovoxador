...
class TextureExporter:
    def __init__(self, cfg):
        self.config = self.get_config(
            cfg['DENSEPOSE_CONFIG'], cfg['DENSEPOSE_WEIGHTS_URL'])
    ...
    def hide_boxes(self, camera, frame, player_tracks, ball_detections, image, use_sift=False):
        ...
        track_bboxes = [track['box'].astype(int)
                        for track in player_tracks["tracks"]]
    ...
    def combine_textures(self, background, transformed_img, alpha=1):
        ...
        background = background.astype(
            np.float32) * (1 - mask) + transformed_img.astype(np.float32) * mask

        return background.astype(np.uint8)

    def extract_pitch(self, background_image, original_image, transformed_image):
        background = self.prepare_pitch_texture(
            background_image, original_image, transformed_image)
    ...
    def get_pitch_texture(self, camera, homography, use_sift=False):
        ...
        final = np.zeros((height, width, channels),
                         dtype=camera.frames[0].dtype)

        for i in tqdm(range(len(camera.frames)), desc='Processing frames'):
            final = self.hide_boxes(
                camera, camera.frames[i], player_tracks[i], ball_detections[i], final, use_sift)

        transformed_img = cv2.warpPerspective(
            final, homography, (m_width, m_height))
    ...
    def get_pitch_texture_lama(self, frame, mask, homography):
        ...
        transformed_output = cv2.warpPerspective(
            output, homography, (m_width, m_height))
